...
for (p in pattern) {
  ...
  for (i in sampleInts) {
    ...
    # i = 45 is the initially selected example
    (ggscatter(input, 
               x = cor.multi.Rs[i, 1], 
               y = cor.multi.Rs[i, 2], 
               add = "reg.line", conf.int = TRUE, 
    ...
    ggsave(paste0("./data/graphics_out/", 
                  prfx, 
                  "_MAUPCorls_",  
                  cor.multi.Rs[i, 1], "_", cor.multi.Rs[i, 2], 
                  "_", sName, ".svg"),
           device = "svg",
           width = grDevices::dev.size(units = "in")[1],
           height = grDevices::dev.size(units = "in")[2],
...
# ---------------------------------------------------------------------------- #
# F. Identifying "flip" examples ----
# ---------------------------------------------------------------------------- #

### Calculate multiple correlations ----
## Spearman correlations between all genes
## For each spot as individual
cor.input <- SA_counts_Nor %>%
  dplyr::select(all_of(select))
...
## For aggregated spots using mean expression
...
# ---------------------------------------------------------------------------- #
melted_all <- reshape2::melt(cor.multi, value.name = "R_all")
melted_361 <- reshape2::melt(cor.multi_361, value.name = "R_361")
melted_layer <- reshape2::melt(cor.multi_layer, value.name = "R_layer")

melted_Rs <- melted_all %>%
  left_join(melted_361) %>%
  left_join(melted_layer)

examples <- melted_Rs %>%
  mutate(filtAll = R_all > 0,
         filt361 = R_361 > 0.1,
         filtLayer = R_layer < -0.7) %>%
  filter(filtAll & filt361 & filtLayer) %>% 
  select(!c("filtAll", "filt361", "filtLayer"))

pattern <- grep("361_mean|layer_mean|input$", names(.GlobalEnv), value = TRUE)

## Selected gene pair is:
# gene_x <- "ENSG00000110484"
# gene_y <- "ENSG00000111341"
# index for that pair in the examples dataframe is 4
index <- 4
...
for (p in pattern) {
  ...
  for (i in (1:nrow(examples))[index]) {
    sName <- gsub("cor.input_", "", p)
    print(i)
    
    x <- as.numeric(unlist(input[,examples[i, 1]]))
    y <- as.numeric(unlist(input[,examples[i, 2]]))
    ...
    sp <- ggscatter(input, 
                    x = as.character(examples[i, 1]), 
                    y = as.character(examples[i, 2]), 
                    add = "reg.line", conf.int = TRUE,
                    size = 3,
                    xlab = "", 
                    ylab = "", 
